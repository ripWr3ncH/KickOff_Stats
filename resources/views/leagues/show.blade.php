                        ...
                        <tr class="border-b border-gray-700 hover:bg-gray-800 transition-colors">
                            <td class="px-4 py-3">
                                <span class="flex items-center justify-center w-6 h-6 text-sm font-bold
                                    @if($index + 1 <= 4) bg-green-600 text-white
                                    @elseif($index + 1 <= 6) bg-orange-600 text-white
                                    @elseif($index + 1 >= count($standings) - 2) bg-red-600 text-white
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-3">
                                    <img src="{{ $standing['team']->logo ?? '/images/default-logo.png' }}" 
                                         alt="{{ $standing['team']->name }}" 
                                         class="w-8 h-8 rounded-full">
                                    <span class="text-light font-medium">{{ $standing['team']->name }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['played'] }}</td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['wins'] }}</td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['draws'] }}</td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['losses'] }}</td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['goals_for'] }}</td>
                            <td class="px-4 py-3 text-center text-light">{{ $standing['goals_against'] }}</td>
                            <td class="px-4 py-3 text-center text-light">
                                <span class="@if($standing['goal_difference'] > 0) text-green-400 @elseif($standing['goal_difference'] < 0) text-red-400 @endif">
